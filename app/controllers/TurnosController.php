...
class TurnosController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'turnos';
		$fields = array('value', 
			'label');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('turnos.value', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title =get_lang('turnos');
		$this->view->render('turnos/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('turnos.value' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title =get_lang('vis_o');
			$this->view->render('turnos/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		...
		$this->view->render('turnos/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->view->page_title =get_lang('editar_turnos');
		if(!empty($data)){
			$this->view->render('turnos/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('turnos.value' , $rec_id,"=",'OR');
		}
