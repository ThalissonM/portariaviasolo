<?php 
/**
 * Unidades Page Controller
 * @category  Controller
 */
class UnidadesController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'unidades';
		$fields = array('id', 
			'nomeunidade');
		...
		if(!empty($this->search)){
			...
			$db->where("(id LIKE ? OR nomeunidade LIKE ?)", array("%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('unidades.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title =get_lang('unidades');
		$this->view->render('unidades/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('unidades.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title =get_lang('vis_o');
			$this->view->render('unidades/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'unidades';
			$fields = $this->fields = array('nomeunidade'); //insert fields
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
				'nomeunidade' => 'required',
			);
			$this->sanitize_array = array(
				'nomeunidade' => 'sanitize_string',
			);
		...
		$this->view->render('unidades/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'unidades';
		$fields = $this->fields = array('id','nomeunidade'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title =get_lang('editar_unidades');
		if(!empty($data)){
			$this->view->render('unidades/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('unidades.id' , $rec_id,"=",'OR');
		}
