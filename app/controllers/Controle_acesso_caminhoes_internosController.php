<?php 
/**
 * Controle_acesso_caminhoes_internos Page Controller
 * @category  Controller
 */
class Controle_acesso_caminhoes_internosController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'controle_acesso_caminhoes_internos';
		$fields = array('id',
			'motorista', 
			'placa', 
			'entrada', 
...
if (!empty($this->search)) {
			...
			if (count($dateRange) === 2) {
				...
				$db->where("(id LIKE ? OR motorista LIKE ? OR placa LIKE ? OR entrada LIKE ? OR saida LIKE ? OR users_id LIKE ? OR nome_responsavel_entrada LIKE ? OR users_id1 LIKE ? OR nome_responsavel_saida LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"));
			}
		}
/*		if(!empty($this->search)){
			$text = trim($this->search);
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('controle_acesso_caminhoes_internos.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->render('controle_acesso_caminhoes_internos/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('controle_acesso_caminhoes_internos.id' , $rec_id);
		}
...
$record['saida'] = format_date($record['saida'],'d/m/Y H:i');
			...
			$this->view->render('controle_acesso_caminhoes_internos/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'controle_acesso_caminhoes_internos';
			$fields = $this->fields = array('motorista','placa','saida','users_id1','nome_responsavel_saida'); //insert fields
			...
			$modeldata['users_id1']=USER_ID;
$modeldata['nome_responsavel_saida']=USER_NAME;
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("controle_acesso_caminhoes_internos");
						}
					}
					return;
				}
		...
		$this->view->render('controle_acesso_caminhoes_internos/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'controle_acesso_caminhoes_internos';
		$fields = $this->fields = array('id','motorista','placa','entrada','saida'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title =get_lang('editar_controle_acesso_caminhoes_internos');
		if(!empty($data)){
			$this->view->render('controle_acesso_caminhoes_internos/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('controle_acesso_caminhoes_internos.id' , $rec_id,"=",'OR');
		}
