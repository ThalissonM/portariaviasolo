<?php 
/**
 * Controle_acesso_caminhoes_chorume Page Controller
 * @category  Controller
 */
class Controle_acesso_caminhoes_chorumeController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'controle_acesso_caminhoes_chorume';
		$fields = array('id',
			'motorista', 
			'placa', 
			'entrada', 
...
/*		if(!empty($this->search)){
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('controle_acesso_caminhoes_chorume.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->render('controle_acesso_caminhoes_chorume/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('controle_acesso_caminhoes_chorume.id' , $rec_id);
		}
...
$record['saida'] = format_date($record['saida'],'d/m/Y H:i');
			...
			$this->view->render('controle_acesso_caminhoes_chorume/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'controle_acesso_caminhoes_chorume';
			$fields = $this->fields = array('motorista','placa','entrada','users_id','nome_responsavel_entrada'); //insert fields
...
$modeldata['nome_responsavel_entrada']=USER_NAME;
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("controle_acesso_caminhoes_chorume");
						}
					}
					return;
				}
		...
		$this->view->render('controle_acesso_caminhoes_chorume/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->view->page_title =get_lang('editar_controle_acesso_caminhoes_chorume');
		if(!empty($data)){
			$this->view->render('controle_acesso_caminhoes_chorume/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('controle_acesso_caminhoes_chorume.id' , $rec_id,"=",'OR');
		}
