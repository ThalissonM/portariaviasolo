<?php 
/**
 * Tipos_veiculos Page Controller
 * @category  Controller
 */
class Tipos_veiculosController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'tipos_veiculos';
		$fields = array('value', 
			'label');
		...
		if(!empty($this->search)){
			...
			$db->where("(value LIKE ? OR label LIKE ?)", array("%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('tipos_veiculos.value', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title =get_lang('tipos_veiculos');
		$this->view->render('tipos_veiculos/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('tipos_veiculos.value' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title =get_lang('vis_o');
			$this->view->render('tipos_veiculos/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'tipos_veiculos';
			$fields = $this->fields = array('value','label'); //insert fields
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("tipos_veiculos");
						}
					}
					return;
				}
		...
		$this->view->render('tipos_veiculos/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'tipos_veiculos';
		$fields = $this->fields = array('value','label'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title =get_lang('editar_tipos_veiculos');
		if(!empty($data)){
			$this->view->render('tipos_veiculos/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('tipos_veiculos.value' , $rec_id,"=",'OR');
		}
