<?php 
/**
 * Controle_acesso_veiculos_leves Page Controller
 * @category  Controller
 */
class Controle_acesso_veiculos_levesController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'controle_acesso_veiculos_leves';
		$fields = array('id', 
			'motorista', 
			'placa',
'observacao', 
...
/*		if(!empty($this->search)){
			$text = trim($this->search);
			$db->where("(id LIKE ? OR colaboradores_id_viasolo LIKE ? OR motorista LIKE ? OR placa LIKE ? OR modelo LIKE ? OR entrada LIKE ? OR saida LIKE ? OR users_id LIKE ? OR nome_responsavel_entrada LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"));
		}*/
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('controle_acesso_veiculos_leves.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
...
$record['saida'] = format_date($record['saida'],'d/m/Y H:i');
		...
		$this->view->page_title =get_lang('controle_acesso_veiculos_leves');
		$this->view->render('controle_acesso_veiculos_leves/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
...
'observacao', 
		...
		if( !empty($value) ){
			...
			$db->where('controle_acesso_veiculos_leves.id' , $rec_id);
		}
...
$record['saida'] = format_date($record['saida'],'d/m/Y H:i');
			...
			$this->view->render('controle_acesso_veiculos_leves/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'controle_acesso_veiculos_leves';
			$fields = $this->fields = array('colaboradores_id_viasolo','motorista','placa','modelo','observacao','entrada','users_id','nome_responsavel_entrada'); //insert fields
...
$modeldata['nome_responsavel_entrada']=USER_NAME;
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("controle_acesso_veiculos_leves");
						}
					}
					return;
				}
		...
		$this->view->render('controle_acesso_veiculos_leves/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'controle_acesso_veiculos_leves';
		$fields = $this->fields = array('id','colaboradores_id_viasolo','motorista','placa','modelo','observacao','entrada','saida','users_id','nome_responsavel_entrada'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
...
$modeldata['nome_responsavel_entrada']=USER_NAME;
		...
		$this->view->page_title =get_lang('editar_controle_acesso_veiculos_leves');
		if(!empty($data)){
			$this->view->render('controle_acesso_veiculos_leves/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
...
'observacao' => 'sanitize_string',
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('controle_acesso_veiculos_leves.id' , $rec_id,"=",'OR');
		}
