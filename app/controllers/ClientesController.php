 ...
 * @category  Controller
 */
class ClientesController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'clientes';
		$fields = array('id', 
			'nome_empresa', 
			'permissao');
		...
		if(!empty($this->search)){
			...
			$db->where("(id LIKE ? OR nome_empresa LIKE ? OR permissao LIKE ?)", array("%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('clientes.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title =get_lang('clientes');
		$this->view->render('clientes/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('clientes.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title =get_lang('vis_o');
			$this->view->render('clientes/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'clientes';
			$fields = $this->fields = array('nome_empresa'); //insert fields
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
				'nome_empresa' => 'required',
			);
			$this->sanitize_array = array(
				'nome_empresa' => 'sanitize_string',
			);
		...
		$this->view->render('clientes/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'clientes';
		$fields = $this->fields = array('id','nome_empresa','permissao'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title =get_lang('editar_clientes');
		if(!empty($data)){
			$this->view->render('clientes/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('clientes.id' , $rec_id,"=",'OR');
		}
