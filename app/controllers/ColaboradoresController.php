<?php 
/**
 * Colaboradores Page Controller
 * @category  Controller
 */
class ColaboradoresController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'colaboradores';
		$fields = array('id_viasolo', 
			'nome');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		$getdata = $this->getdata; //array of sanitized values passed via $_GET;
		if(!empty($this->search)){
			$text = trim($this->search);
			$db->where("(id_viasolo LIKE ? OR nome LIKE ? OR rg LIKE ? OR permissao LIKE ?)", array("%$text%","%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('colaboradores.id_viasolo', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title =get_lang('colaboradores');
		$this->view->render('colaboradores/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('colaboradores.id_viasolo' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title =get_lang('vis_o');
			$this->view->render('colaboradores/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'colaboradores';
			$fields = $this->fields = array('id_viasolo','nome'); //insert fields
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("colaboradores");
						}
					}
					return;
				}
		...
		$this->view->render('colaboradores/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'colaboradores';
		$fields = $this->fields = array('id_viasolo','nome'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title =get_lang('editar_colaboradores');
		if(!empty($data)){
			$this->view->render('colaboradores/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('colaboradores.id_viasolo' , $rec_id,"=",'OR');
		}
